...
struct HHawkes <: ParametricModel
    ...
    HHawkes(I::Tuple{Real, Real}) = new(convert_interval(I))
    HHawkes(a::Real, b::Real) = HHawkes((a, b))
end
...
function estimate(model::HHawkes, events::Events; step_tol::Float64 = 1e-3, max_iter::Int = 500)
    N             = length(events)
    ...
    while (error >= step_tol) && (iter < max_iter) # Stop iteration when optimization is smaller then tolerance 
        ...
        error = max(μ - (D / N), ψ - (D / div), β - (D / div))
        μ, ψ, β = D / N,  D / div, D / div # Update parameters for the next iteration
    end
...
function time_transform(model::HHawkes, params::Parameters{3}, events::Events)
    diffs     = diff([model.I[1]; events; model.I[2]])
    N         = length(events)
    A         = zeros(N + 1) # Array A in Ozaki (1979)
    @inbounds for i in 2:N+1
        A[i] = exp(-params[3] * diffs[i]) * (1 + A[i-1])
    end
    transf   = (params[2] / params[3]) * ((0:N-1) - (@view A[1:N]))
    transf .+= time_transform(params[1], events, model.I)[1] # Integral of base rate
    return transf, (params[1] * measure(model.I)) + ((params[2] / params[3]) * (N - A[end]))
end
...
function time_transform!(model::HHawkes, params::Parameters{3}, events::Events)
    ...
    events .= time_transform(params[1], events, model.I)[1] # Integral of base rate
    @inbounds for i in eachindex(events)
        events[i] += (params[2] / params[3]) * ((i - 1) - A[i])
    end
    return (params[1] * measure(model.I)) + ((params[2] / params[3]) * (N - A[end]))
end
...
# function ∫CIF(params::ParametersHH,
#                 rec::Record,
#                 _...)
...
function min_max_CIF(model::HHawkes, params::Parameters{3}, events::Events)
    M = 0
    for i in eachindex(events)
        temp = sum(exp.(-params[3] * (events[i] - events[1:i])))
        M = (temp > M) ? temp : M
    end
    return params[1], params[1] + (params[2] * M)
end


# function simulate!(model::HHawkes, params::Parameters{3}, events::Events)
#     N_base = simulate!(params[1], model.I, events)
#     N_desc = generate_descendants!(events, N_base, model.I[2], params[2], params[3])
#     return N_desc
# end
